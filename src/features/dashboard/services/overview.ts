  ...
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const getCostOverTime = Effect.fn("getCostOverTime")(function* () {
        const rows = yield* Effect.try({
          try: () =>
            ...
            new OverviewError({ cause, message: "Failed to get cost over time" }),
        });
      ...
      const getModelUsage = Effect.fn("getModelUsage")(function* () {
        const rows = yield* Effect.try({
          try: () =>
            db.select({ models: sessionSummary.models, totalCost: sessionSummary.totalCost }).from(sessionSummary).all(),
      ...
      const getTopProjects = Effect.fn("getTopProjects")(function* () {
        const projectRows = yield* Effect.try({
          try: () =>
            db.select({ id: project.id, name: project.name }).from(project).all(),
      ...
      const getDashboardMetrics = Effect.fn("getDashboardMetrics")(function* () {
        const [cards, costOverTime, modelUsage, thinkingLevels, stopReasons] =
          yield* Effect.all(
            [
              getCards(),
              getCostOverTime(),
