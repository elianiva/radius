	...
	static readonly layer = Layer.effect(
		...
		Effect.gen(function* () {
			...
			const yearFilter = (year?: number) =>
				year
					? sql`strftime('%Y', ${sessionSummary.createdAt} / 1000, 'unixepoch') = ${String(year)}`
					: undefined;
			...
			const getAll = Effect.fn("getWrappedData")(function* (year?: number) {
				...
				const swearRows = yield* Effect.try({
					try: () => {
						...
						if (filter)
							q.where(
								sql`strftime('%Y', ${swearEntry.createdAt} / 1000, 'unixepoch') = ${String(year)}`,
							);
						return q
				...
				const topSwear =
					swearRows.length > 0
						? { word: swearRows[0]!.topWord, count: swearRows[0]!.topCount }
						: null;
				...
				const busiestDay = busyRows[0]
					? { date: busyRows[0].date, count: busyRows[0].count }
					: null;
				...
				const modelJourneyRows = yield* Effect.try({
					try: () => {
						...
						if (filter) q.where(filter);
						return q
							.orderBy(sql`strftime('%Y-%m', ${sessionSummary.createdAt} / 1000, 'unixepoch')`)
							.all();
					},
				...
				const toolRows = yield* Effect.try({
					try: () => {
						const q = db
							...
							.from(event)
							.where(
								inArray(event.sessionId, sessionIds).append(
									sql`and ${event.eventType} = 'message'`,
								),
							);
						return q.all();
					},
				...
				const thinkingRows = yield* Effect.try({
					try: () => {
						...
						return q
							.groupBy(sql`json_extract(data, '$.thinkingLevel')`)
							.orderBy(sql`count(*) desc`)
							.all();
					},
				...
				const worstErrorDay =
					errorDayRows[0] && errorDayRows[0].errorCount > 0
